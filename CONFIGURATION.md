...
- **`KB_MEMORY_THRESHOLD`** (config key `memory_threshold`, default `0.45`; resolved per call via `retrieve_params` since TASK-188, with `MEMORY_MIN_COS` in `kb-recall.py` as the offline-tooling fallback): the memory layer's own floor. Memories are short and atomic, so their cosine against a prompt sits structurally lower than a wiki article's. Measured on `qwen3-embedding:4b` over 1224 memory questions, 806 of them retrievable in the top 5: min 0.340, p10 0.484, p50 0.615. A `0.60` floor discarded 366 of those 806; `0.45` discards 42. The previous `0.60` was not merely stale after the model switch: re-measured against an 8b index it discarded 260 of 798 there too, so it was over-filtering by a third on the model it was chosen for. Set it to `0.0` for rank-only measurement.
...
- **Storage**: local SQLite only. Tables: `activity_events` (entities, topics
  and artifacts live in its JSON columns and in `search_blob`) and
  `source_watermarks`. Earlier versions also carried `activity_entities`,
  `activity_topics`, `activity_artifacts`, an FTS5 table `activity_fts` and a
  `rollup_cache`; all are dropped on the next index build. The first four were
  never read; the rollup cache was, but cost more than it saved and keyed on too
  few fields. Topic matching runs as a substring comparison over `search_blob`.
