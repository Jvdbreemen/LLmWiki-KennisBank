...
_MCP_SELECTION: list[str] | None = None
_MCP_PROBE = Path(__file__).with_name("_mcp_probe.py")


def _candidate_python_argv() -> list[list[str]]:
    ...
    for env_name in ("VIRTUAL_ENV", "CONDA_PREFIX"):
        ...
        if root:
            name = "python.exe" if _is_windows_like() else "python"
            candidates.append([str(Path(root) / "bin" / name if not _is_windows_like()
                                   else Path(root) / name)])
    candidates.append(_agent_python_argv())
...
def _probe_python(argv: list[str], timeout: int = 15) -> dict:
    try:
        proc = subprocess.run(
            [*argv, str(_MCP_PROBE), "--json"],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
        ...
        return {"ok": False, "error_code": "interpreter_missing",
                "error": f"interpreter not found: {_shell_join(argv)}"}
    except subprocess.TimeoutExpired:
        return {"ok": False, "error_code": "probe_timeout",
                "error": f"runtime probe timed out: {_shell_join(argv)}"}
    ...
    if proc.returncode != 0 or not result.get("ok"):
        ...
        result.setdefault("error_code", "probe_failed")
        detail = result.get("error") or (proc.stderr or "").strip() or f"probe exit {proc.returncode}"
        result["error"] = detail
    result["argv"] = argv
    return result


def select_capable_interpreter(timeout: int = 15) -> list[str]:
    """Select an interpreter that can import MCP and execute a vec0 query."""
    global _MCP_SELECTION
    if _MCP_SELECTION is not None:
        return list(_MCP_SELECTION)
    failures: list[str] = []
    for candidate in _candidate_python_argv():
        result = _probe_python(candidate, timeout=timeout)
        if result.get("ok"):
            _MCP_SELECTION = list(candidate)
            return list(_MCP_SELECTION)
        failures.append(
            f"{_shell_join(candidate)}: {result.get('error_code', 'probe_failed')}"
            f" ({result.get('error', 'onbekend')})"
        )
    attempted = ", ".join(_shell_join(c) for c in _candidate_python_argv())
    raise RuntimeError(
        "geen Python-interpreter kan zowel mcp als sqlite-vec/vec0 laden. "
        f"Pogingen: {attempted}. Details: {'; '.join(failures)}. "
        "Installeer in een geschikte virtual environment met "
        "`mcp==1.28.1` en `sqlite-vec==0.1.9`, en zet "
        "KENNISBANK_PYTHON naar die interpreter; voer daarna setup opnieuw uit."
    )


def _mcp_server_argv(vault: Path) -> list[str]:
    return [*select_capable_interpreter(), _posix(vault / ".claude" / "scripts" / "kb-mcp.py")]
...
def _write_opencode_plugin(path: Path, vault: Path) -> Path:
    ...
    vault_s = _posix(vault)
    python = _shell_join([_mcp_server_argv(vault)[0]])
...
async function run(script) {{
  try {{
    await $`${{python}} ${{scripts}}/${{script}}`.env({{
      ...process.env,
...
def install_copilot(repo: Path, vault: Path) -> dict:
    ...
    report = _copilot.install(vault, python_argv=_mcp_server_argv(vault)[:-1])
    res = report["results"]
    return {
        "skills": [str(p) for p in skills],
        "mcp": res["mcp"]["path"],
...
def validate_mcp_runtime(vault: Path, timeout: int = 15) -> list[str]:
    ...
    try:
        server_argv = _mcp_server_argv(vault)
    except Exception as exc:
        return [str(exc)]
    py = server_argv[:-1]
    if not py:
        return ["KennisBank MCP interpreter selection returned an empty command"]
...
'''
    ...
    try:
        ...
        return [f"KennisBank MCP handshake timed out: {_shell_join(server_argv)}"]
    if proc.returncode != 0:
        detail = "\n".join(x for x in (proc.stderr.strip(), proc.stdout.strip()) if x).strip()
        if len(detail) > 1200:
            detail = detail[:1200] + "..."
        if "No module named mcp" in detail or "No module named 'mcp'" in detail:
            return [
                "KennisBank MCP dependency missing or broken for "
                f"{_shell_join(py)}; run: {_shell_join([*py, '-m', 'pip', 'install', 'mcp==1.28.1', 'sqlite-vec==0.1.9'])}"
                f" ({detail})"
            ]
        return [f"KennisBank MCP handshake failed for {_shell_join(server_argv)}: {detail}"]
    return []
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        if result.get("mcp_python"):
            print(f"KennisBank MCP interpreter: {result['mcp_python']}")
