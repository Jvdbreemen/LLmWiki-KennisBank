...
- Prefer the local KennisBank MCP server before external search when a task may depend on prior local knowledge. For an explicit question about what worked before, use `experience_recall` first; use `source_recall` only on demand for underlying evidence, reconstruction, or verification. Neither deeper route is an automatic advisory. For temporal questions use `what_did_i_do`, `timeline`, `weeklog` or `topic_timeline` first. Use `shortest_path` for an explicit graph connection question.
...
- `shortest_path(source, target, max_hops)` for an explicit graph connection question.
