...
Half done: the buffered-stdout fix shipped in a1af2c7 and is verified in the file; the Job Object failing to reap children on an external stop remains open, reproduced three times. This is a genuine Windows-specific investigation, not an hour job. The strongest lead is recorded in the code itself: the fail-open branch at atlas/launch.py:99-103 returns None silently, so the guard may never have engaged when the launcher ran inside a session-manager's own job — instrumenting that return value is the first step. The reproduction details (ports, PIDs, TaskStop behavior) live in this task's description and comment; the Job Object design rationale is in the docstring at atlas/launch.py:26-36 and the fd5df04 commit.

**Evidence:** Commit a1af2c7 'fix(atlas): flush launch.py's print()s so a redirected log shows the URL' (AC#1 shipped, verified flush=True at atlas/launch.py:141,147,162); Job Object at atlas/launch.py:26-104 (commit fd5df04, TASK-85) with a silent fail-open at :99-103; grep of atlas/README.md finds no known-limitation/manual-cleanup note, so AC#3 is unmet.

**Remaining work (when reopened):** ACs #2/#3: reproduce the orphaned sidecar/vite children under a real Ctrl-C (does the Job Object clean up there?), then find why an external stop leaves them alive. First check: log whether _windows_kill_on_close_job() returned a handle at all — AssignProcessToJobObject fails open silently at launch.py:99-103, and a launcher already inside the harness's own job without breakaway rights would return None with no trace (hypothesis, unverified). Fix, or document as a known limitation with a manual-cleanup note in atlas/README.md.
