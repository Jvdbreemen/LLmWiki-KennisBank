...
| **Location (nominal)** | `adapters/` (repo-relative) |
...
- **`.github/copilot-instructions.md`** (2 lines, whole file) — the `push-inject` payload. Its exact
  ...
  user-level Copilot instructions block written by `scripts/_copilot.py:553`.
...
### 2.3 `scripts/install-agent-envs.py` — the cross-agent installer and validator (1170 lines)

Role: the single entry point `setup.sh` calls to install and validate the non-Claude harnesses and to
validate the Claude deploy. Stdlib only. Called from `setup.sh:215`, `:219`, `:249`, `:254`
...
|---|---|---|
| `AGENTS = ("claude", "codex", "opencode", "copilot")` | `:40` | The closed set of harness ids. `parse_agents` rejects anything else. |
| `KB_START`, `KB_END` | `:50-51` | HTML-comment markers delimiting the managed block in freeform files. |
| `ROOT_COMMANDS: dict[str, str]` | `:53-70` | 16 command stems → description; the source list for prompts/skills. |
| `NESTED_COMMAND_ALIASES: dict[str, str]` | `:72-76` | `kennisbank/<x>` → flat `kennisbank-<x>` alias for harnesses without nested command names. |
| `MODEL_CHECK_TEXT` | `:78` | Smoke-test prompt; the model must answer `OK`. |
| `OPENROUTER_ENDPOINT`, `OPENROUTER_DEFAULT_MODEL` | `:79-80` | The only non-local provider path. |

**Public / entry-point functions — full signatures**

- `install_codex(repo: Path, vault: Path) -> dict` — `:271`
...
- `install_opencode(repo: Path, vault: Path) -> dict` — `:488`
...
- `install_copilot(repo: Path, vault: Path) -> dict` — `:588`
  ...
  paths plus the resolved Copilot home. Depends on `scripts/_copilot.py:593`.

- `validate_files(repo: Path, vault: Path, agents: list[str]) -> list[str]` — `:627`
  The hard gate. Returns a list of error strings; empty means pass.
  Always checks 10 deployed vault files (`:629-640`: `kb-mcp.py`, `kb-retrieve.py`,
  ...
  Then per harness: Claude `:644-698`, Codex `:700-762`, OpenCode `:764-775`, Copilot `:777-789`
  ...
  *Observed coverage gap (not a crash, worth knowing):* the Claude branch hardcodes four script
  names at `:665-672` instead of iterating `_hooks_manifest.hooks()`. `script_names` is derived from
  the manifest at `:656` but is used only to select entries for the `statusMessage` check. So
...
- `configure_llm(vault: Path, provider: str, model: str | None = None, api_key_env: str = "OPENROUTER_API_KEY", api_key_value: str | None = None) -> dict` — `:922`
  Writes `<vault>/.claude/kennisbank-llm.json`. `provider` must be `ollama` or `openrouter`; anything
  else raises `ValueError` (`:950`). An OpenRouter key is stored out-of-band by `_write_user_secret`.

- `parse_agents(raw: str | None) -> list[str]` — `:1086`
  `None`/empty → `["claude", "codex"]`; `all` → all four; unknown ids → `SystemExit` (`:1093`).

- `main(argv: list[str] | None = None) -> int` — `:1098`
  ...
  `validation_errors` rather than raised (`:1146-1147`).

**Helper functions — summarized, not dropped.** All 30 private helpers, with signature and line:

| Signature | Location | Role |
|---|---|---|
| `_norm_path(raw: str | Path) -> Path` | `:83` | Expands `~`/env vars; rewrites Git Bash `/d/...` to `D:/...` on Windows. |
| `_posix(p: Path) -> str` | `:93` | Backslashes → forward slashes for generated config. |
| `_is_windows_like() -> bool` | `:97` | `nt`, `win*`, `msys`, `cygwin`. |
| `_agent_python_argv() -> list[str]` | `:101` | `["py","-3"]` on Windows-like, else `["python3"]`. |
| `_mcp_server_argv(vault: Path) -> list[str]` | `:105` | Interpreter + `<vault>/.claude/scripts/kb-mcp.py`. The single definition of the MCP command. |
| `_shell_join(argv: list[str]) -> str` | `:109` | Platform-correct quoting (`shlex.quote` on POSIX). |
| `_home() -> Path` | `:123` | `USERPROFILE` → `HOME` → `Path.home()`. |
| `_codex_home() -> Path` | `:128` | `CODEX_HOME` if non-blank, else `~/.codex`. The `.strip()` guard is deliberate (`:129-131`): an empty env var would otherwise resolve to `Path(".")`. |
| `_opencode_home() -> Path` | `:136` | `OPENCODE_CONFIG_DIR` else `~/.config/opencode`. |
| `_read_text(path: Path) -> str` | `:141` | `""` on missing file. |
| `_write_text(path: Path, text: str) -> None` | `:148` | mkdir -p + UTF-8 write. |
| `_copytree(src: Path, dst: Path) -> None` | `:153` | `dirs_exist_ok=True` skill copy. |
| `_replace_block(path: Path, block: str) -> bool` | `:158` | Marker-scoped managed block; appends when absent. Returns changed. |
| `_agent_block(client: str, vault: Path) -> str` | `:172` | The AGENTS.md text: pins `KENNISBANK_VAULT`, forbids the `~/KennisBank` default, states the fail-open rule. |
| `_command_sources(repo: Path) -> list[tuple[str, Path, str]]` | `:193` | `(name, source .md, description)` for root commands and nested aliases. |
| `_prompt_text(name, source, description, target_agent) -> str` | `:206` | Frontmatter + vault instruction + the command body, for Codex/OpenCode prompts. |
| `_command_skill_text(name, source, description) -> str` | `:220` | Same, as a `SKILL.md`. |
| `_install_shared_skills(repo: Path, skills_root: Path) -> list[Path]` | `:236` | Copies `repo/skills/*/SKILL.md` trees. |
| `_install_command_skills(repo: Path, skills_root: Path) -> list[Path]` | `:250` | Generates command skills, but a hand-authored skill of the same name always wins (`:257-261`). |
| `_codex_command(script: str, vault: Path, event: str) -> str` | `:296` | Builds a Codex hook command; coordinators get `--client codex`, silent scripts route through `quiet-hook.py`. |
| `_hook_group(script, vault, event, matcher=None, timeout=60) -> dict` | `:315` | One Codex hook group. |
| `_ensure_codex_hooks(path: Path, vault: Path) -> None` | `:334` | Prunes legacy SessionStart/Stop entries, de-duplicates the coordinator, then upserts the desired map. Raises `ValueError` on a malformed file. |
| `_is_kennisbank_hook_command(command: str) -> bool` | `:447` | Ownership test: the command must contain `.claude/scripts/` *and* a known KennisBank script basename. |
| `_toml_quote(value: str) -> str` | `:458` | TOML string escaping. |
| `_ensure_codex_mcp(path: Path, vault: Path) -> None` | `:462` | Regex-replaces or appends the `[mcp_servers.kennisbank]` + `.env` blocks in `config.toml`. |
| `_write_opencode_plugin(path: Path, vault: Path) -> Path` | `:513` | Emits `kennisbank.js`: on `session.idle` runs 6 index/notify scripts; on `session.updated` runs `archive-transcript.py` and `kb-usage-scan.py`; every call wrapped in try/catch (fail-open). |
| `_ensure_opencode_config(path, vault, plugin) -> Path` | `:559` | Key-scoped merge of `mcp.kennisbank`, sets `autoupdate: false`, allows 3 skills by default. |
| `_hook_entries(data: dict)` | `:609` | Generator over every hook entry dict in a settings/hooks document. |
| `_json_file(path: Path) -> dict` | `:885` | Fail-open JSON read → `{}`. |
| `_secrets_path() -> Path` | `:895` | `KENNISBANK_SECRETS_FILE` else `~/.config/kennisbank/secrets.json`. |
| `_write_user_secret(name: str, value: str) -> Path` | `:902` | Writes the secret and chmods `0600` (best effort). |
| `_read_user_secret(name: str) -> str` | `:914` | Env var first, then the secrets file. |
| `_resolve_llm_config(vault: Path) -> dict` | `:955` | Merges `kennisbank-llm.json` with `KB_LLM_*` env overrides. |
| `_resolve_embed_config(vault: Path) -> dict` | `:969` | Same for `kennisbank-embed.json` / `KB_EMBED_*`. |

### 2.4 `scripts/_copilot.py` — the Copilot config adapter (811 lines)

Role: the hermetically testable Copilot layer that implements ADR-0003 D1–D6. Stdlib only, no
module-level `sys.path` mutation, so tests can import it standalone. Has its own CLI.
...
| `BACKUP_SUFFIX = ".kbak"` | `:201` | One rolling backup per freeform file. |
| `_CAPTURE_SCRIPT = "kb-copilot-capture.py"` | `:317` | Registered once, referenced by three events. |
| `_LEGACY_SESSION_START` | `:318-327` | 8 script names pruned from `sessionStart` on upgrade. |
| `_MANAGED_HOOK_SCRIPTS` | `:328-333` | Ownership set used by `_remove_hooks`. |
| `_LEGACY_SESSION_END` | `:334-338` | 3 script names pruned from `sessionEnd`. |

**Public functions — full signatures**

Detection:
- `copilot_home() -> Path` — `:92`. `COPILOT_HOME` if non-blank, else `~/.copilot`. This override is what makes the whole Copilot test suite hermetic.
- `find_binary() -> "str | None"` — `:109`. `KENNISBANK_COPILOT_BIN` overrides `shutil.which("copilot")`.
- `binary_version(binary: "str | None" = None, timeout: int = 20) -> "tuple | None"` — `:123`. Runs `copilot --version`; returns `None` on the Windows/nvm4w "no platform package found" case.
- `detect(vault: "Path | None" = None) -> dict` — `:144`. JSON-serializable snapshot: 13 keys including `installed`, `version_ok`, `platform_binary_ok`, `kennisbank_registered`, `hooks_present`. Note the `vault` parameter is accepted but not used in the body.

Idempotent primitives (ADR D6):
- `replace_managed_block(path: Path, block: str, *, dry_run: bool) -> dict` — `:226`. Marker-scoped rewrite of a freeform file; backs up first; returns a result dict.
- `merge_json_key(path: Path, top_key: str, name: str, value: dict, *, dry_run: bool) -> dict` — `:247`. `data[top_key][name] = value` with an equivalence check, so a repeat run reports `skipped`.
- `remove_json_key(path: Path, top_key: str, name: str, *, dry_run: bool) -> dict` — `:271`.
- `remove_managed_block(path: Path, *, dry_run: bool) -> dict` — `:283`.

Surface writers:
- `ensure_mcp(home: Path, vault: Path, *, dry_run: bool = False) -> dict` — `:308`. Writes `mcpServers.kennisbank` into `<home>/mcp-config.json` with `type: "local"`, `command`/`args` from `_mcp_server_argv`, literal `env`, `tools: ["*"]`.
- `ensure_hooks(home: Path, vault: Path, *, dry_run: bool = False) -> dict` — `:436`. Writes `<home>/hooks/kennisbank.json` as `{"version":1,"hooks":{<event>:[entry...]}}`. Prunes legacy entries, enforces one coordinator per lifecycle event, then upserts by `(script, arg)` so re-runs never duplicate and unrelated user entries survive.
- `ensure_instructions(home: Path, vault: Path, *, dry_run: bool = False) -> dict` — `:543`. Managed block in `<home>/copilot-instructions.md`.
- `ensure_agent_profile(home: Path, vault: Path, *, dry_run: bool = False) -> dict` — `:579`. Writes `<home>/agents/kennisbank.agent.md`. If the file exists **without** the KennisBank marker it is left untouched and reported `skipped` (`:601-604`). The `.agent.md` extension is mandatory (`:597-598`).

Orchestration and validation:
- `install(vault: Path, *, home: "Path | None" = None, dry_run: bool = False) -> dict` — `:593`. Runs the four `ensure_*` writers; returns `{home, vault, dry_run, results, changed}`.
- `remove(vault: Path, *, home: "Path | None" = None, dry_run: bool = False) -> dict` — `:612`. Surgical rollback of exactly those four surfaces.
- `validate_config(vault: Path, *, home: "Path | None" = None) -> list` — `:671`. Login-free hard errors: `mcpServers.kennisbank` present, its `env.KENNISBANK_VAULT` equals the active vault, its `args` mention `kb-mcp.py`, each desired hook appears exactly once, no legacy entries, instructions and profile exist.
- `probe_cli(vault: Path, *, home: "Path | None" = None, timeout: int = 25) -> dict` — `:722`. Runs `copilot mcp list` under an explicit `COPILOT_HOME` and classifies the outcome as one of `copilot_missing`, `platform_binary_missing`, `mcp_list_failed`, `not_logged_in`, `mcp_not_listed`, `version_old`, `ok`.
- `_main(argv=None) -> int` — `:774`. CLI: `{detect,install,remove,probe,validate}` with `--vault`, `--dry-run`, `--json`; output is always JSON. `setup.sh:117` calls `_copilot.py detect --json`. The `probe` branch resolves a missing `--vault` through `_vaultpath.vault_root()` (`:790`), honoring ADR-0002.

**Helpers — summarized, not dropped:** `_kb_env(vault) -> dict` (`:44`, the four pinned env vars including `KENNISBANK_MCP_COMPACT_OUTPUT`),
`_is_windows_like` (`:55`), `_norm_path` (`:59`), `_posix` (`:69`), `_win` (`:73`), `_home` (`:77`),
`_mcp_server_argv` (`:102`), `_version_tuple` (`:118`), `_read_text` (`:174`), `_write_text` (`:181`),
`_read_json` (`:186`, fail-open → `{}`), `_write_json` (`:197`), `_backup` (`:204`),
`_result(path, action, backed_up=None, detail="") -> dict` (`:215`, the
`created|updated|skipped` outcome record that replaces a bare bool — ADR D6), `_mcp_server_spec`
(`:297`), `_hook_timeout(script)` (`:341`, lazily imports `_hooks_manifest` and falls back to
`{240, 90, 30}` so a missing manifest cannot break an install), `_desired_hooks` (`:361`),
`_hook_command` (`:377`), `_hook_entry` (`:409`), `_hook_matches` (`:426`), `_instructions_block`
(`:521`), `_agent_profile_text` (`:549`), `_remove_hooks` (`:623`).

`_hook_command` (`:377-406`) carries the single most important line in the Copilot adapter: it
appends `; exit 0` to every generated command (`:406`) so a KennisBank `preToolUse` hook can never
...
|---|---|---|---|
| C1 | **One MCP server, one name, one path.** Exactly one server named `kennisbank`, command built from a single helper, pointing at `<vault>/.claude/scripts/kb-mcp.py`. | `install-agent-envs.py:105`, `_copilot.py:102` | `install-agent-envs.py:718-721` (Codex: exactly one block), `_copilot.py:685` (args mention `kb-mcp.py`), `validate_mcp_runtime:793` |
| C2 | **Vault pinning, no interpolation.** Every generated config carries a literal `KENNISBANK_VAULT`; no `${VAR}` (Copilot does not interpolate). Scripts resolve the vault only via `_vaultpath.vault_root()` — ADR-0002. | `_copilot.py:44-50`, `install-agent-envs.py:472-475`, `:574-578`, `kennisbank-copilot.py:82-83`, `_copilot.py:790` | `install-agent-envs.py:673` (Claude env), `:727-730` (Codex), `_copilot.py:683` (Copilot vault match) |
| C3 | **Interpreter convention.** `py -3` on Windows-like platforms, `python3` elsewhere; existing prefixes are preserved on re-registration. | `install-agent-envs.py:101`, `_copilot.py:102`, `register-hooks.py:42`, `:146` | `tests/test_register_hooks.py:35-37` (interpreter selection per platform), `:50-56` (stale path refreshed while the `py -3` prefix survives) |
| C4 | **Idempotent, key-scoped, non-destructive mutation.** Structured config → merge one namespaced key with an equivalence check. Freeform files → marker-delimited managed block with a `.kbak` backup. Unmanaged content is never rewritten. | `_copilot.py:247` (`merge_json_key`), `:226` (`replace_managed_block`), `:204` (`_backup`), `install-agent-envs.py:158`, `:559` | `_copilot.py:601-604` (unmarked user profile left intact); ADR-0003 D6 (`docs/adr/0003-copilot-cli-integration.md:204-221`); `tests/test_agent_envs_install.py:229` (Copilot install idempotent), `:161` (MCP repair does not duplicate the env subtable), `tests/test_copilot_e2e.py:120` (Copilot install does not regress Codex) |
| C5 | **Exactly one start coordinator and one exit coordinator per harness.** Legacy fan-out scripts must be pruned during upgrade, then replaced by the coordinator. | `register-hooks.py:187-236`, `install-agent-envs.py:345-397`, `_copilot.py:452-490` | `install-agent-envs.py:693-694` (Claude), `:742-743`, `:755-756` (Codex), `_copilot.py:698-709` (Copilot); `tests/test_agent_envs_install.py:92` (Codex consolidation), `tests/test_hooks_manifest.py:46-54` (one `SessionEnd` coordinator + the legacy set) |
...
| C9 | **Runtime proof, not just file presence.** MCP registration is accepted only after a real `initialize()` + `list_tools()` handshake finds the required names and call-smokes ordinary, source, and experience recall without an MCP error. | `install-agent-envs.py:validate_mcp_runtime` | Runs whenever Codex, OpenCode, or Copilot is selected; unit and wire tests cover missing dependency, handshake failure, tool calls, and default-off fail-open responses. |
...
|---|---|---|---|---|---|---|
| **Claude Code** | `~/.claude/skills/<name>/SKILL.md` (installed by `setup.sh`, validated at `install-agent-envs.py:649-651`) | `~/.claude/commands/*.md` (`:646-648`) | `CLAUDE.md` in the vault | `~/.claude/settings.json` via `register-hooks.py:172`; events `SessionStart`, `UserPromptSubmit`, `SessionEnd`, `PreToolUse`, `PreCompact` | **not registered by this layer** — Claude gets KennisBank through the hooks path. `agent-status.py:40` hardcodes `mcp: False` and `install-agent-envs.py:1142` runs the handshake only for codex/opencode/copilot. A user may of course have registered `kennisbank` in their own Claude MCP config; nothing here writes or reads it. | — |
| **Codex CLI** | `~/.agents/skills/<name>/SKILL.md` (`install-agent-envs.py:271`) | `~/.codex/prompts/<name>.md`, invoked as `/prompts:<name>` (`:279-282`) | `~/.codex/AGENTS.md` managed block (`:284`) | `~/.codex/hooks.json` (`:285`); events `SessionStart`, `UserPromptSubmit`, `Stop`, `PreToolUse` — note **`Stop`**, not `SessionEnd` (`:401-416`), and no `PreCompact` | `~/.codex/config.toml`, `[mcp_servers.kennisbank]` (`:286`, `:462`) | `CODEX_HOME` |
| **OpenCode** | `~/.agents/skills/` (shared) (`:494`) | `~/.config/opencode/commands/<name>.md` — real slash commands (`:496-499`) | `~/.config/opencode/AGENTS.md` managed block (`:501`) | `~/.config/opencode/plugins/kennisbank.js`, driven by `session.idle` / `session.updated` (`:513-554`) | `~/.config/opencode/opencode.json`, `mcp.kennisbank` (`:559`) | `OPENCODE_CONFIG_DIR` |
| **GitHub Copilot CLI** | `~/.agents/skills/` — shared, **no separate install** (`:595-596`) | exposed as slash commands from the shared skills | `~/.copilot/copilot-instructions.md` managed block (`_copilot.py:543`) **and** the opt-in repo-local `.github/copilot-instructions.md` from `adapters/registry.json` | `~/.copilot/hooks/kennisbank.json` (`_copilot.py:436`); events `sessionStart`, `userPromptSubmitted`, `preToolUse`, `postToolUse`, `sessionEnd` (`:361-374`) | `~/.copilot/mcp-config.json`, `mcpServers.kennisbank` (`_copilot.py:308`) | `COPILOT_HOME` |
| **Copilot extras** | — | — | custom agent profile `~/.copilot/agents/kennisbank.agent.md` (`_copilot.py:579`) | launcher `kennisbank-copilot.py`, capture `kb-copilot-capture.py`, import `import-copilot.py` | — | `KENNISBANK_COPILOT_BIN` for the binary |
...
| `scripts/install-agent-envs.py` | `<vault>/.claude/scripts/kb-mcp.py` and 9 other deployed files (`:629-640`) | Validation targets. |
| `scripts/_copilot.py` | `scripts/_hooks_manifest.py` (lazy, `:351`), `scripts/_vaultpath.py` (lazy, `:790`) | Timeout ceilings; ADR-0002 vault resolution. Both lazy so the module stays standalone-importable. |
...
(`install-agent-envs.py:800`, `:832-859`; remediation hint at `:818`).
...
- `ollama` — `ollama list`, `ollama show <model>` (`install-agent-envs.py:1008`, `:989`).
- `py` / `python3` — the harness-side interpreter written into every generated command.
- `bun` — implied by the generated OpenCode plugin's `import { $ } from "bun"` (`install-agent-envs.py:517`).

**HTTP endpoints.**
- `http://localhost:11434/api/embeddings` and `/api/generate` — local Ollama smoke tests (`install-agent-envs.py:1029`, `:1041`).
- `http://localhost:11434` — the `KB_LLM_ENDPOINT` pinned into every generated config.
- `https://openrouter.ai/api/v1/chat/completions` — only when `openrouter` is an explicitly configured provider (`install-agent-envs.py:79`, `:1066`). This is the sole non-local network call in the layer.
...
flowchart TD
    ...
    IAE --> ICODEX["install_codex :271"]
    IAE --> IOC["install_opencode :488"]
    IAE --> ICOP["install_copilot :588"]
    IAE --> VF["validate_files :627"]
    IAE --> VMCP["validate_mcp_runtime<br/>initialize + list_tools + call<br/>ordinary + source + experience"]
    IAE --> VMOD["validate_models :978"]
...
3. **`validate_files` under-covers the Claude hook manifest.** Four script names are hardcoded at
   `install-agent-envs.py:665-672`; `kb-session-end-recover.py` and `kb-checkpoint.py` from
...
4. **`SILENT_HOOK_SCRIPTS` is an empty frozenset** (`_hooks_manifest.py:53`), so the
   `quiet-hook.py` routing branch in `_codex_command` (`install-agent-envs.py:296-304`) is currently
   unreachable for Codex. Copilot still uses `quiet-hook.py` unconditionally for its non-coordinator
   hooks (`_copilot.py:386-400`).
