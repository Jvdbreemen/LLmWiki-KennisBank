...
| `kb-experience.db` | `<vault>/.claude/kb-experience.db` | `rebuild-experience.py` (staged rebuild) and append-only event capture | `kb-experience-recall.py`, doctor, evaluation | Typed event/outcome evidence plus derived validated experiences and optional local vector/FTS projection |
