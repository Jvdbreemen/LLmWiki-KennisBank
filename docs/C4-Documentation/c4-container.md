...
- Experimental evidence projections — [c4-component-retrieval.md](./c4-component-retrieval.md): `build-source-index.py`, `kb-source-recall.py`, `rebuild-experience.py`, `kb-experience-recall.py`, and `kb-projection-doctor.py`; all off-hot-path and opt-in
...
| `kb-source.db` | `<vault>/.claude/kb-source.db` | `build-source-index.py` (staged full/incremental rebuild) | `kb-source-recall.py`, doctor, evaluation | Disposable provenance-first RAG projection: source chunks, hashes, offsets, safe scalar metadata; raw files remain authoritative |
| `kb-experience-ledger.db` | `<vault>/.claude/kb-experience-ledger.db` | Append-only capture and owner reviews | `rebuild-experience.py`, doctor | Canonical typed event, outcome, and review evidence; retained as the audit source of truth |
| `kb-experience-index.db` | `<vault>/.claude/kb-experience-index.db` | `rebuild-experience.py` (full staged rebuild) | `kb-experience-recall.py`, doctor, evaluation | Disposable projection of accepted, source-grounded experiences using local hybrid retrieval or lexical fallback |

(A further store, `kb-usage.db`, backs `_usage.py`'s injected/used/noise tracking and Atlas's warmth ranking. It remains separate from both experimental projections.)

**Concurrency contract**: "Only sweep and index builds write to `kb-index.db`; retrieval reads only" — the single rule that lets §2.1 (hot path) and §2.3 (background workers) coexist without lock contention. Experimental builders write only their own staged databases. All stores are local files; none are network-accessible.
...
- **MCP tool count**: source (`scripts/kb-mcp.py`) now confirms **eleven** `@mcp.tool()`-decorated functions: ordinary recall, explicit source/experience recall, capture/review, graph `shortest_path`, and the four temporal tools. The install gate requires the nine graph/retrieval/temporal tool names; both deeper routes remain opt-in and fail-open. Older prose saying six, eight, or ten tools is historical and must not be used as the current contract.
- **`kb-usage.db`**: referenced by name in the Retrieval and Atlas component docs but never given its own schema/ownership description in any of the nine documents. This container document treats it as part of the `kb-index.db`/usage-tracking family rather than a fourth top-level SQLite store, since no source evidence was found describing it as independently schema-owned; resolved (TASK-208): `kb-usage.db` is a fourth top-level store, owned and schema-managed by `scripts/_usage.py` (`DB_NAME`, `_SCHEMA`, `_migrate`); see the Retrieval component doc for the schema.
