...
- **MCP tool count**: source (`scripts/kb-mcp.py`) now confirms **ten** `@mcp.tool()`-decorated functions: the original eight plus explicit `source_recall` and `experience_recall`. The install gate requires the eight read-only/write retrieval primitives and temporal tools; both experimental routes remain opt-in and fail-open. Older prose saying six or eight tools is historical and must not be used as the current contract.
