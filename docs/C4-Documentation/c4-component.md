...
|---|---|---|
| `kb-index.db` (vector + FTS, `vec_docs`/`docs`) | Index builders (`build-kb-index.py`, `build-embed-index.py`, `embed-sweep.py`) and Memory Lifecycle (`_memory.py` → `upsert()` for verified memories); "only sweep and index builds write to kb-index.db" | Retrieval hot path (read-only, per documented concurrency constraint), Memory Lifecycle (`_groundcheck.py`/`_reconcile.py` via `search()`/cosine), Atlas (read-only SQL, `?mode=ro`), Quality Assurance (hermetic exercise) |
